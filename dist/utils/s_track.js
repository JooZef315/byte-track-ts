...
export class STrack extends BaseTrack {
    constructor(xywh, score, cls) {
        super();
        this.kalman_filter = null;
        this.mean = null; // 8-dim
        this.covariance = null; // 8x8
        this.angle = null;
        this.tracklet_len = 0;
    ...
    static multi_predict(stracks) {
        if (stracks.length <= 0)
            return;
        const multi_mean = [];
        const multi_covariance = [];
        for (const track of stracks) {
            if (!track.mean || !track.covariance)
                continue;
        ...
        if (multi_mean.length === 0)
            return;
        const res = STrack.shared_kalman.multi_predict(multi_mean, multi_covariance);
        let idx = 0;
    ...
    re_activate(new_track, frame_id, new_id = false) {
        ...
        const res = this.kalman_filter.update(this.mean, this.covariance, this.convert_coords(new_track.tlwh));
        this.mean = res.mean;
        this.covariance = res.covariance;
        this.tracklet_len = 0;
        this.state = TrackState.Tracked;
    ...
    update(new_track, frame_id) {
        ...
        const res = this.kalman_filter.update(this.mean, this.covariance, this.convert_coords(new_tlwh));
        this.mean = res.mean;
        this.covariance = res.covariance;
        this.state = TrackState.Tracked;
        this.is_activated = true;
...
STrack.shared_kalman = new KalmanFilterXYAH();

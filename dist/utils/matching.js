 ...
 */
export function linear_assignment(cost_matrix, thresh) {
    if (!cost_matrix ||
    ...
    for (let i = 0; i < rows; i++) {
        ...
        for (let j = 0; j < cols; j++) {
            const v = row[j];
            if (Number.isFinite(v) && v > maxCost)
                maxCost = v;
        }
    }
    ...
    const n = rows + cols;
    const augmented = Array.from({ length: n }, () => Array(n).fill(bigCost));
...
export function iou_distance(atracks, btracks) {
    let atlbrs = [];
    let btlbrs = [];
    if (atracks.length && btracks.length) {
        ...
        if (arraysProvided) {
            ...
            if (atlbrs[0] && atlbrs[0].length === 5 && btlbrs[0]?.length === 5) {
                const ious = batch_probiou(atlbrs, btlbrs);
                return ious.map((row) => row.map((val) => 1 - val));
            }
            const ious = bbox_ioa(atlbrs, btlbrs, true);
            return ious.map((row) => row.map((val) => 1 - val));
        }
...
export function fuse_score(cost_matrix, detections) {

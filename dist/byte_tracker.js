...
export class BYTETracker {
    ...
    init_track(results, feats = null) {
        if (!results || !results.conf || results.conf.length === 0)
            return [];
    ...
    // Helpers
    static joint_stracks(tlista, tlistb) {
    ...
    update(results, img = null) {
        this.frame_id += 1;
        const activated_stracks = [];
        const refind_stracks = [];
        const lost_stracks = [];
        const removed_stracks = [];
        const scores = results.conf;
        const remain_inds = [];
        const inds_low = [];
        const inds_high = [];
        for (let i = 0; i < scores.length; i++) {
            if (scores[i] >= this.args.track_high_thresh)
                remain_inds.push(i);
            if (scores[i] > this.args.track_low_thresh)
                inds_low.push(i);
            if (scores[i] < this.args.track_high_thresh)
                inds_high.push(i);
        }
        ...
        const strack_pool = BYTETracker.joint_stracks(tracked_stracks, this.lost_stracks);
        ...
        for (const [itracked, idet] of matches) {
            if (itracked === undefined || idet === undefined)
                continue;
            const track = strack_pool[itracked];
            const det = detections[idet];
            if (!track || !det)
                continue; // Safety guard for undefined access
        ...
        let matches2, u_track2;
        if (r_tracked_stracks.length === 0) {
            matches2 = [];
            u_track2 = [];
        }
        else {
            const dists2 = matching.iou_distance(r_tracked_stracks, detections_second);
        ...
        let matches3, u_unconfirmed, u_detection3;
        ...
        for (const inew of u_detection3) {
            ...
            if (track.score < this.args.new_track_thresh)
                continue;
        ...
        this.tracked_stracks = this.tracked_stracks.filter((t) => t.state === TrackState.Tracked);
        this.tracked_stracks = BYTETracker.joint_stracks(this.tracked_stracks, activated_stracks);
        this.tracked_stracks = BYTETracker.joint_stracks(this.tracked_stracks, refind_stracks);
        this.lost_stracks = BYTETracker.sub_stracks(this.lost_stracks, this.tracked_stracks);
        this.lost_stracks.push(...lost_stracks);
        this.lost_stracks = BYTETracker.sub_stracks(this.lost_stracks, this.removed_stracks);
        const [filtered_tracked, filtered_lost] = BYTETracker.remove_duplicate_stracks(this.tracked_stracks, this.lost_stracks);
        ...
        if (this.removed_stracks.length > 1000) {
            this.removed_stracks = this.removed_stracks.slice(this.removed_stracks.length - 1000);
        }
        return this.tracked_stracks
            .filter((x) => x.is_activated)
            .map((x) => x.result);
    }
    slice_results(results, indices) {
        ...
        const ret = { xywh: new_xywh, conf: new_conf, cls: new_cls };
        if (results.xywhr) {
